...
public class requete9 {
    private static String INPUT_PATH = "input/";
    private static String OUTPUT_PATH = "input-tmp/req9Join-";
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            // on Joueur -> id,nom,prénom,pseudo,solde,type,email,date_naissance,age,adresse,ville,pays,haut_faits,coord_bancaire
                ...
                if (!words[0].equals("id")) {
                    ...
                    if (age < 31) {
                        context.write(new Text("[18-30]"), new Text(words[0]));
                    } else if (age < 51) {
                        context.write(new Text("[31-50]"), new Text(words[0]));
                    } else if (age < 71) {
                        context.write(new Text("[51-70]"), new Text(words[0]));
                    } else {
                        context.write(new Text("[71-etc]"), new Text(words[0]));
                    }
                }
            }
        }
