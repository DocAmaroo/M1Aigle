...
public class requete5hash {
    private static String INPUT_PATH = "input/";
    private static String OUTPUT_PATH = "input-tmp/req5Joinhash-";
    ...
    private static HashMap<String,Boolean> IDJoueurAllow = new HashMap<String, Boolean>();
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            // on Date -> idDate,Horodatage
                ...
                if (!words[0].equals("idDate")) {
                    ...
                    hash.put(words[0],split[1] + "/" + split[2]);
                }
            }
            // on Joueur -> id,nom,prénom,pseudo,solde,type,email,date_naissance,age,adresse,ville,pays,haut_faits,coord_bancaire
                ...
                try{
                    IDJoueurAllow.put(words[0],Boolean.parseBoolean(words[5]));
                }catch (Exception e){}
            }
        }
    }
    ...
    public static class Reduce extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context)
            ...
            boolean vip = false;
            for (Text val : values) {
                if (IDJoueurAllow.get(val.toString())){
                    vip = true;
                    cpt++;
                }
            }
            if (vip)
                context.write(new Text(hash.get(key.toString())), new Text(","+cpt+",[X]"));
        }
    }
