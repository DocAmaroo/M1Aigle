...
to setup
  ...
  ;; creation d'un but. vous pouvez ajouter vos but et obstacles ...
...
;; Strucutre de base du noeud de recherche : le patch concerne,
;; une valeur pour l'algorithme et le noeud parent (qui a genere ce noeud)
;; A Modifier en fonction des algorithmes...
to-report make-node [ptch val parent]
 report (list ptch val parent)  ;; !! prend en compte une valeur associée au noeud
end
...
to search-step
  ;; liste de recherche : ajoute les noeuds generes a la liste courante et trie la liste selon un critere
  set in-nodes sort-nodes merge-nodes generate in-nodes

  let proposed first in-nodes
  if (proposed = []) [ stop ] ;; liste vide => tout visite => on arrête

  set current proposed

  set in-nodes bf in-nodes
  set out-nodes (fput (get-patch current) out-nodes) ;; on ne stock que les patches dans la liste de sortie
...
;; A Modifier en fonction des algorithmes...
to-report generate
let aset ([neighbors] of (get-patch current)) with [obstacle = 0] ;; on prend tout les voisins valide
  ifelse (any? aset) [
    ask aset [set pcolor white] ;; colorer des patches ajoutes a la liste de recherche
    ...
    [
      ...
      [ x -> make-node
        x
        dist-patches mygoal x;; choisir un critere
        current
      ]
    ...
    [
      ...
      [ x -> make-node
        x
        (get-val current + 1);; choisir un critere
        current
      ]
      lst1 ;; make a list of nodes [patch value]
      report result
...
to-report merge-nodes [lst-new lst-ref]
  ...
  [ elem-new ->
    ...
    foreach lst-ref
    [ elem-ref ->
      if get-patch elem-new = get-patch elem-ref
      [set cnt cnt + 1]
    ]
    if cnt = 0
