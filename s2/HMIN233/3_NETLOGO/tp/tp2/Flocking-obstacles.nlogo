...
turtles-own [
  obstacles          ;; agentset des obstacles voisins
...
to go
  ...
  []
  [
    ask turtles [ flock fd 0.5 count-collisions]
  ]
  tick
end
...
to find-obstacles
  set obstacles patches in-radius distance-to-avoid-obstacles with [pcolor = red]
end

to flee
  let obstacles-in-front obstacles in-cone angle-avoidance angle-flee
...
to-report vect-flee
  let obstacles-in-front min-one-of obstacles in-cone angle-avoidance angle-flee [distance myself]
  if obstacles-in-front != nobody
  [
    report vectFromAngle (towards obstacles-in-front + 180) 10
  ]
  report vectFromAngle heading 1
end
...
to-report vect-direction
  let Va multiplyScalarvect facteur-align vect-align
  let Vs multiplyScalarvect facteur-separe vect-separate
  let Vc multiplyScalarvect facteur-cohere vect-cohere
...
;;; HELPER PROCEDURES
;; limite la rotation de l'agent a max-turn
;; plus realiste: cela prend plusieurs tick pour faire demi-tour
...
;;; FONCTIONS DE GESTION DE VECTEURS
...
;; Netlogo n'est pas dans le repère classique: le 0 est vers le
;; haut et les angles tournent vers la droite. De ce fait,
;; x = sin angle dans ce référentiel...
...
to-report orthoVect1 [angle len]
  let v vectFromAngle angle len
  let xc (- item 1 v)
  let yc item 0 v
  report (list xc yc)
end

to-report orthoVect2 [angle len]
  let v vectFromAngle angle len
  let xc item 1 v
  let yc (- item 0 v)
  report (list xc yc)
