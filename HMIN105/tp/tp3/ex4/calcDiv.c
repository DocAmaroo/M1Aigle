...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        int rcv_res = (msgrcv(msgid, &requete, sizeof(requete.contenu), 5, 0));
        printf("calcDiv - nb octect reçu : %i\n", rcv_res);
        ...
        response.result = (double) requete.contenu.op1 / (double) requete.contenu.op2;
        printf("resultt : %f", response.result);
